...
//		String date=this.jumin.substring(2, 4)+"월 "+this.jumin.substring(4, 6)+"일";
		// String값이 07이면 int형으로 변환하면 > 7 
		// 앞자리가 0이면 정수니까 없어진다. 07이렇게는 문자. > int는 7
		
		return Integer.parseInt(this.jumin.substring(2,4));
...
//		return date;
	...
	public int getDate(){
		return Integer.parseInt(this.jumin.substring(4,6));
	}
...
//		System.out.println(animal);
	...
	public int getHistory(){
		...
		Calendar start=Calendar.getInstance();	//현재날짜
		Calendar end=Calendar.getInstance();	//태어난날짜.
		
		
		
		start.set(getYear(),getMonth()-1,getDate());
	...
	public void disp(){
		...
		if(validate()){
			...
			System.out.println("생년월일 : "+String.valueOf(getYear())+"년 "+getMonth()+"월 "+getDate()+"일");//정수형 맞고, 만약 더하고 싶으면
			                                                                                               //괄호를 잘 묶자. 안그럼 뒤에 String에 영향을 받는다.
			System.out.println("나이 : "+String.valueOf(getAge())+"살");
			System.out.println("띠 : "+getAni());
			System.out.println("성별 : "+getSex());
			System.out.println("태어난 일 수 : "+getHistory()+"일");
//			showSex();
		}else{
